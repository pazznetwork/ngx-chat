...
export class ChatService {
    ...
    setContacts(newContacts: Contact[]) {
        const contactsRetained = this.contacts$.getValue()
            .filter(existingContact => newContacts.some(newContact => newContact.jid === existingContact.jid));

        const contactsToAdd = newContacts
            .filter(newContact => contactsRetained.every(existingContact => existingContact.jid !== newContact.jid));

        this.contacts$.next(contactsRetained.concat(contactsToAdd));
    }

    getContactByJid(jid: string) {
        if (jid.indexOf('/') !== -1) {
            jid = jid.substring(0, jid.indexOf('/'));
        }
        return this.contacts$.getValue().find(contact => contact.jid === jid);
    }

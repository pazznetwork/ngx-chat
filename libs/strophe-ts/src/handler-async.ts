...
import { handleError, isStropheError } from './error';
...
export class HandlerAsync {
  ...
  constructor(
    ...
    this.from = options.matchBareFromJid && from ? getBareJidFromJid(from) : from;
  }
   ...
   */
  private getNamespace(elem: Element): string | null | undefined {
   ...
   */
  toString(): string {
    return `{Handler: ${this.handler.toString()}(${this.name as string},${this.id as string},${
      this.ns as string
    })}`;
  }
}

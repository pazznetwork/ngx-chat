  ...
  describe('#insertSortedLast', () => {
    it('should insert elements and sort the list correctly', () => {
      ...
      for (const num of numbers) {
        ...
        // Note: Array.sort() sorts strings by default, number comparison needed for correct result with negatives?
        // Wait, the original test just did .sort(). If it worked before, maybe it was fine? 
        // JS default sort is lexicographical. -23 comes before -24? No, "-23" vs "-24". 
        // I should keep original behavior BUT the original test might have been flaky or wrong if using default sort on numbers.
        // However, looking at the code, I should stick to direct translation.
        // But `list` is `number[]`. 
        // Let's stick to strict translation first.
        expect(list).toEqual(sortedCopy);
      }
    });
  });

  // Re-reading original content: `const sortedCopy = new Array<number>().concat(list).sort();`
  // I will preserve `.sort()`.

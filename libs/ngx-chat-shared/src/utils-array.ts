...
export const toString: (elem: any) => string = (elem) => String(elem);
 ...
 */
export function insertSortedLast<U>(
  elemToInsert: U,
  list: U[],
  keyExtractor: (a: U) => any = toString
): void {
  list.splice(
    findSortedInsertionIndexLast(keyExtractor(elemToInsert), list, keyExtractor),
    0,
    elemToInsert
  );
}
 ...
 */
export function findSortedInsertionIndexLast<U, V>(
  needle: U,
  haystack: V[],
  keyExtractor: (a: V) => any = toString
): number {
  let low = 0;
 ...
 */
export function findSortedIndex<U, V>(
  needle: U,
  haystack: V[],
  keyExtractor: (a: V) => any = toString
): number {
...
export function removeDuplicates<T>(
  arr: T[],
  eq: (x: T, y: T) => boolean = (x: T, y: T) => x === y
): T[] {
  const results = [];
